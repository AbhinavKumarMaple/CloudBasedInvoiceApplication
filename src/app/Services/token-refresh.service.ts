...
export class TokenRefreshService {
  ...
  refreshAccessTokenCustomer() {
    ...
    if (expirationTime) {
      ...
      if (!isExpired) {
        this.http.get<MyResponse>(`https://invoice-backend-nodejs-production-up.up.railway.app/api/accountant/refresh-token`, { observe: 'response', withCredentials: true }).subscribe(response => {
          if (response.body) {
  ...
  refreshAccessTokenEmployee() {
    ...
    if (expirationTime) {
      ...
      let isExpired = new Date(currentDate) > new Date(expirationTime);
      if (!isExpired) {
        this.http.get<MyResponse>(`https://invoice-backend-nodejs-production-up.up.railway.app/api/employee/refresh-token`, { observe: 'response', withCredentials: true }).subscribe(response => {
          if (response.body) {
            localStorage.setItem('expirationTime', response.body.expirationTime)
          }
        });
